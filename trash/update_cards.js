...
// The new unified card component string.
// We replace the internal rendering of the map callback.
const getNewCard = (itemName) => `
                <Card key={${itemName}.id} className="!bg-transparent !border-none shadow-none cursor-pointer group rounded-3xl p-4 card-hover-breathe relative overflow-hidden">
                  ...
                  <CardContent className="p-0 sm:p-4 text-center relative z-10">
                    ...
                    <p className="text-xs text-gray-400 mb-4 font-light tracking-wide opacity-80 group-hover:opacity-100 transition-opacity duration-300">
                    ...
                    {${itemName}.email && (
                      <div className="flex justify-center mb-3">
                        <a
                          ...
                          className="p-1.5 text-gray-500 hover:text-indigo-300 transition-colors"
                    ...
                    <Button
                      variant="outline"
                      size="sm"
                      onClick={() => onMemberClick?.(${itemName})}
                      className="relative z-10 rounded-full backdrop-blur-md bg-white/5 border border-white/10 text-white/80 px-6 py-1 h-auto text-xs font-medium tracking-wide transition-all duration-500 overflow-hidden group-hover:bg-indigo-500/10 group-hover:text-white group-hover:border-indigo-400/30 group-hover:shadow-[0_0_15px_rgba(99,102,241,0.2)]"
                    >
                      <span className="relative z-10">{t('team.viewProfile')}</span>
                    </Button>
                  </CardContent>
                </Card>
`;
